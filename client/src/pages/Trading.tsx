...
export default function Trading() {
  ...
  const positionSize = useMemo(() => {
    ...
    const distance = Number(stopDistance) / 100;
    const entry = orderType === "limit" ? Number(price) : currentPrice;
    if (!Number.isFinite(budget) || budget <= 0 || !Number.isFinite(distance) || distance <= 0 || !Number.isFinite(entry) || entry <= 0) return null;
    return budget / (entry * distance);
  }, [currentPrice, orderType, price, riskBudget, stopDistance]);
